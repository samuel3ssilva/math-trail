...
| T3 | **XSS via imported JSON** (malicious backup with HTML in `notes`/`activity`) | L | M | Import validates schema and enum fields before touching storage; unknown activities render as plain ids; notes render inside text nodes via template strings — residual risk documented, sanitization is the next hardening step |
